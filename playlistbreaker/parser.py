...
from playlistbreaker.data_models import Track, TracklistFormat
...
def parse_description(link: str, tracklist_format: TracklistFormat):
        ...
        if match:
            ...
            tracks.append(
                Track(
                    ...
                    start=int(track_info.get("secs", 0))
    ...
    if not len(line_wise_desc):
        logger.warning(
            f"Did not found any tracks with the format {tracklist_format.value} in the description."
        )
    return tracks
